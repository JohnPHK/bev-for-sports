...
The MySQL image is based on `mysql:8.0`. The following command copies `bev4sports.sql` into the `/docker-entrypoint-initdb.d/` directory during the Docker image build. This script is automatically executed to set up the MySQL database schema.

### bev4sports Application Image Setup

The bev4sports application image is based on `tomcat:9.0-jdk8-corretto`.
> **Note:**  
> The URL `bev4sports-sql:3306/bev4sports` is used **inside the Docker container**, but the code and the WAR file may use different configurations.  
> As a result, the connection string in `DBConnection.java` differs depending on the context.
...
// Used in the WAR file and local development
connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/bev4sports", "root", "password");

```

Finally, the WAR file is copied to `/usr/local/tomcat/webapps` of the image so the tomcat server image can host the web application.
